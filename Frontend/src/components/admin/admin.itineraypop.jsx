...
const AdItineryPop = ({ isOpen, onClose, onSave }) => {
  ...
  const [selectedItemDest, setSelectedItemDest] = useState(null);
  const [itemFilteredDestinations, setItemFilteredDestinations] = useState([]);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setItemSearch("");
      setSelectedItemDest(null);
    }
  }, [isOpen, baseUri, token]);
  ...
  const handleItemSearchChange = (e) => {
    ...
    if (value.trim().length > 0) {
      ...
      setItemFilteredDestinations(filtered);
  ...
  const submitStepTwo = async (e) => {
    e.preventDefault();
    if (!selectedItemDest) {
      toast.error("Please select a destination for this activity.");
      return;
    }
    setLoading(true);
    ...
    >
      <div className="relative overflow-hidden">
        <div className="flex w-full h-1.5 bg-gray-900 overflow-hidden">
          <div
            className={`h-full bg-emerald-500 transition-all duration-700 ease-in-out ${step === 1 ? "w-1/3" : "w-full"}`}
          />
        </div>
        ...
        >
          ...
          >
              ...
              <div
                ...
                className={`relative group w-full h-40 rounded-2xl border-2 border-dashed transition-all flex flex-col items-center justify-center cursor-pointer overflow-hidden ${
                  previewUrl
                    ...
                    : "border-gray-800 hover:border-gray-600 bg-gray-900/20"
                }`}
              >
                {previewUrl ? (
                  <>
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              <div className="relative">
                <FaSearch
                  className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors ${validDestination ? "text-emerald-500" : "text-gray-700"}`}
                />
                <input
                  name="title"
                  autoComplete="off"
                  ...
                  className={`${inputClass} pl-10 ${validDestination ? "border-emerald-500/50" : ""}`}
              ...
              {showDropdown && (
                <ul className="absolute z-[100] w-full mt-2 bg-gray-950 border border-gray-800 rounded-xl shadow-2xl max-h-52 overflow-y-auto no-scrollbar backdrop-blur-xl ring-1 ring-white/5">
                  {filteredDestinations.length > 0 ? (
                    filteredDestinations.map((dest) => (
                      <li
                        key={dest.id}
                        onClick={() => selectDestination(dest)}
                        className="group px-4 py-3 text-sm text-gray-400 hover:bg-emerald-600 hover:text-white cursor-pointer transition-all flex justify-between items-center border-b border-gray-900 last:border-0"
                      >
                    ...
                    <li className="px-4 py-4 text-center text-xs text-gray-600 italic">
              ...
              <p className="text-[9px] text-gray-500 mt-1 ml-1">
                ⚠️ Use suggestions to ensure valid database linking
              </p>
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-4 bg-emerald-600 hover:bg-emerald-500 rounded-2xl font-bold text-white shadow-xl shadow-emerald-900/20 transition-all flex items-center justify-center gap-3 disabled:opacity-50"
            >
          ...
          >
            ...
            <div className="relative">
              ...
              <div className="relative">
                <FaSearch
                  className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors ${selectedItemDest ? "text-emerald-500" : "text-gray-700"}`}
                />
                <input
                  autoComplete="off"
                  required
                  ...
                  className={`${inputClass} pl-10 ${selectedItemDest ? "border-emerald-500/50" : ""}`}
                  placeholder="Search location for this activity..."
                />
                {selectedItemDest && (
                  <FaCheck className="absolute right-4 top-1/2 -translate-y-1/2 text-emerald-500 text-xs" />
                )}
              ...
              {showItemDropdown && (
                <ul className="absolute z-[100] w-full mt-2 bg-gray-950 border border-gray-800 rounded-xl shadow-2xl max-h-40 overflow-y-auto no-scrollbar">
                  {itemFilteredDestinations.length > 0 ? (
                    itemFilteredDestinations.map((dest) => (
                      <li
                        key={dest.id}
                        onClick={() => {
            ...
            <div className="relative w-full h-28 rounded-2xl overflow-hidden border border-gray-800 shadow-lg">
              {previewUrl ? (
                ...
                <div className="w-full h-full bg-gray-900 flex items-center justify-center">
                  <FaImage className="text-gray-800" size={30} />
                </div>
              )}
              <div className="absolute inset-0 bg-gradient-to-t from-gray-950 via-gray-950/40 to-transparent flex flex-col justify-end p-4">
                <p className="text-emerald-400 text-[8px] font-black uppercase tracking-[0.2em]">
                  Add Activity to
                </p>
                <h4 className="text-white font-bold text-sm truncate">
                  {formData.title || "New Expedition"}
                </h4>
              </div>
            </div>
            ...
            <div className="space-y-3 pt-2">
              <button
                ...
                className="w-full py-4 bg-emerald-600 hover:bg-emerald-500 rounded-2xl font-bold text-white shadow-xl shadow-emerald-900/20 transition-all flex items-center justify-center gap-3 disabled:opacity-50"
              >
                {loading ? "Finalizing..." : "Publish Activity"}{" "}
                <FaCheck size={12} />
              </button>
              <button
                type="button"
