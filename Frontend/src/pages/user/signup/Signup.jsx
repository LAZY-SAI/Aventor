...
const InputField = ({
  ...
  <div className="relative w-full max-w-xs mb-6">
    ...
    {error && (
      <p className="text-red-500 text-[10px] font-bold uppercase mt-1 absolute">
        {error}
      </p>
    )}
  </div>
  ...
  const validate = (isLogin) => {
    ...
    if (isLogin) {
      ...
      if (formData.signupPassword.length < 6)
        newErrors.signupPassword = "Min 6 characters";
    }
    setErrors(newErrors);
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-slate-100 font-sans p-4">
      ...
      <div className="w-full max-w-[900px] h-[600px] bg-white rounded-[2rem] shadow-2xl relative overflow-hidden flex">
        ...
        >
          <h2 className="text-3xl font-black text-indigo-600 mb-8 uppercase tracking-tighter">
            Register
          </h2>
          <form
        ...
        >
          ...
          <button
            type="button"
            className="mt-4 flex items-center gap-2 text-xs font-bold text-emerald-600 uppercase tracking-widest hover:opacity-70 transition-opacity"
          >
