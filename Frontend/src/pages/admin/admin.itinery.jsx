...
const AdItinery = () => {
  ...
  const filteredItineraries = itineraries.filter((item) => {
    ...
    const matchesStatus =
      statusFilter === "All Status" || itemStatus === statusFilter;
  ...
  const Toggle = ({ isPublic }) => (
    <div className="flex items-center justify-end gap-3">
      <span
        className={`text-[9px] font-black uppercase tracking-widest ${isPublic ? "text-emerald-500" : "text-gray-600"}`}
      >
        {isPublic ? "Live" : "Draft"}
      </span>
      <div className="relative inline-flex items-center">
        <div
          className={`w-10 h-5 rounded-full transition-colors ${isPublic ? "bg-emerald-600/20 border border-emerald-500/30" : "bg-gray-800 border border-gray-700"}`}
        >
          <div
            className={`absolute top-[2px] left-[2px] rounded-full h-4 w-4 transition-transform ${isPublic ? "translate-x-5 bg-emerald-500" : "translate-x-0 bg-gray-500"}`}
          ></div>
        </div>
      </div>
    </div>
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center pb-8 gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 font-medium">
              Curate and deploy world-class trekking experiences
            </p>
          </div>
          <button
                        ...
                        >
                          ...
                          <td className="px-6 py-6 relative">
                            <button
                              onClick={() =>
                                setActiveMenuId(
                                  activeMenuId === itemId ? null : itemId,
                                )
                              }
